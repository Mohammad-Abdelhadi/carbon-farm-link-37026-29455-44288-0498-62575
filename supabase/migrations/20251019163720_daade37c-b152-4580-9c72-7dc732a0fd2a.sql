...
CREATE TYPE public.app_role AS ENUM ('admin', 'farmer', 'investor');
...
CREATE POLICY "Farmers can create their own farms"
  ...
  WITH CHECK (auth.uid() = user_id AND public.has_role(auth.uid(), 'farmer'));

CREATE POLICY "Investors can view all farms"
  ON public.farms FOR SELECT
